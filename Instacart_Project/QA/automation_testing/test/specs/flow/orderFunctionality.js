...
describe('Order functionality', () => {
    describe('Tests for shops', () =>{
        ...
        it('check that products are available after clicking on shop', () => {
            ...
            assert.equal(firstProductText, 'Pasta Spaghetti Divella Private');
        })
    })
    describe('Tests for products', () =>{
        it('check that product name is available', () => {
            const secondProductText = productPageObject.getSecondProductText();
            assert.equal(secondProductText, 'Milk');
        })
        it('check that product price is available', () => {
            const firstProductPriceText = productPageObject.getFirstProductPriceText();
            assert.equal(firstProductPriceText, 'Price: 590.0 AMD');
        })
    })
    ...
    describe('Positive tests for shops, products and card', () =>{
        it('check that products from different shops added to card and cost is correct', () => {
            ...
            productPageObject.clickOnCard();
            const totalCost = productPageObject.getTotalCost();
            assert.equal(totalCost, 'Total Cost 10490');
        })
    })
})